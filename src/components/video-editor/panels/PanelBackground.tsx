...
export default function PanelBackground({ selected, onWallpaperChange }: PanelBackgroundProps) {
	...
	const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (!["image/jpeg", "image/jpg"].includes(file.type)) {
			toast.error(t("imageUpload.invalidFileType"), { description: t("imageUpload.jpgOnly") });
			event.target.value = "";
			return;
		}
		...
		reader.onerror = () =>
			toast.error(t("imageUpload.failedToUpload"), { description: t("imageUpload.errorReading") });
	...
	const handleRemoveCustomImage = (imageUrl: string, event: React.MouseEvent) => {
		...
		if (selected === imageUrl) onWallpaperChange(WALLPAPER_PATHS[0]);
	};
		...
		<div>
			<Tabs defaultValue="image" className="w-full">
				...
				<TabsContent value="image" className="mt-0 space-y-2">
					...
					<div className="grid grid-cols-7 gap-1.5 max-h-[200px] overflow-y-auto custom-scrollbar pr-0.5">
